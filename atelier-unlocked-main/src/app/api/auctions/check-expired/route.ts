...
// This endpoint checks and updates expired auctions
// Can be called from client-side without auth (public check)
export async function POST(request: NextRequest) {
  try {
      ...
      if (winningBid) {
        ...
        if (winningBid.bidder) {
          try {
            ...
            // Skip email if admin access not available
            console.error('Could not send winner email:', e);
          }
        }
        ...
        if (auction.designer) {
          try {
            ...
            // Skip email if admin access not available
            console.error('Could not send designer email:', e);
          }
        }

        soldCount++;
      } else {
        // No bids, mark as VOID instead
        await supabase
          .from('auctions')
          .update({ status: 'VOID' })
          .eq('id', auction.id);
    ...
    return NextResponse.json({
      ...
      processed: {
        sold: soldCount,
        void: (lockedAuctions?.length || 0) + (unlockedAuctions?.length || 0) - soldCount,
      },
    });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error('Check expired auctions error:', error);
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    );
